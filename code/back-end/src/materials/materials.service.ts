...
export class MaterialsService {
	...
	async update(id: string, updateMaterialDto: UpdateMaterialDto): Promise<void> {
	...
	async validateCreate(createMaterialDto: CreateMaterialDto): Promise<void> {
    	...
    	const existingMaterial = await this.materialsRepository.findOne({ where: { code: code.toUpperCase() } });
		if (existingMaterial) {
			throw new ConflictException('Material with this code already exists.');
		}
	}

	async validateUpdate(id: string, updateMaterialDto: UpdateMaterialDto): Promise<Material> {
		const { code } = updateMaterialDto;
    	const existingMaterial = await this.validateExistingMaterial(id);

		const existingMaterialCode = await this.materialsRepository.findOne({ where:  { id: Not(id), code: code.toUpperCase() } });
