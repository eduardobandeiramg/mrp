...
export class BuildOfMaterialsService {
	...
	async create(createBuildOfMaterialDto: CreateBuildOfMaterialDto): Promise<void> {
        const { product, material, parentBuildOfMaterial } = await this.validateCreate(createBuildOfMaterialDto);
		...
		try {
            ...
            throw new InternalServerErrorException('Error while creating Build Of Material');
        }
	}
	...
	async remove(id: string): Promise<void> {
        ...
        try {
            ...
            throw new InternalServerErrorException('Error while deleting Build Of Material');
        }
	}

	async getByProductAndLvl(productId: string, lvl: number): Promise<BuildOfMaterial[]> {
        const product = await this.productsService.validateExistingProduct(productId);
        ...
        try {
            ...
            throw new InternalServerErrorException('Error while retrieving Build of Materials');
        }
    }
    

	async getDirectChildren(id: string): Promise<BuildOfMaterial[]> {
        const parentBuildOfMaterial = await this.buildOfMaterialsRepository.findOne( { where: { id } });
	...
	async validateCreate(createBuildOfMaterialDto: CreateBuildOfMaterialDto): Promise<{ 
        product: Product, 
        material: Material, 
        parentBuildOfMaterial?: BuildOfMaterial 
    }> {
        const { productId, materialId, parentBuildOfMaterialId } = createBuildOfMaterialDto;

        const product = await this.productsService.validateExistingProduct(productId);
        const material = await this.materialsService.validateExistingMaterial(materialId);

        let parentBuildOfMaterial: BuildOfMaterial = null;
        if (parentBuildOfMaterialId) {
            parentBuildOfMaterial = await this.buildOfMaterialsRepository.findOne({ where: { id: parentBuildOfMaterialId } });
            if (!parentBuildOfMaterial) {
                throw new NotFoundException(`Parent BuildOfMaterial with ID ${parentBuildOfMaterialId} not found`);
            }
        }
	...
	async calculateLevel(parentBuildOfMaterial?: BuildOfMaterial): Promise<number> {
        if (parentBuildOfMaterial) {
