...
describe('BuildOfMaterialsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(BuildOfMaterial), useValue: mockBuildOfMaterialsRepository },
    ...
    buildOfMaterialsRepository = module.get<Repository<BuildOfMaterial>>(getRepositoryToken(BuildOfMaterial));
  });

  describe('create', () => {
    it('deve criar e salvar um BuildOfMaterial', async () => {
      const createDto = { productId: '123', materialId: '456', parentBuildOfMaterialId: '789', qtd: 10 };

      const product: Product = { id: '123', description: 'Test Product', code: 'P123', isActive: true };
      const material: Material = { id: '456', description: 'Test Material', code: 'M456', qtd: 100 };
      const parentBuildOfMaterial: BuildOfMaterial = { id: '789', lvl: 1, qtd: 10, product, material, parentBuildOfMaterial: null, childMaterials: [] };
      const buildOfMaterial: BuildOfMaterial = { id: 'abc', product, material, parentBuildOfMaterial, lvl: 2, qtd: 10, childMaterials: [] };

      jest.spyOn(service, 'validateCreate').mockResolvedValue({ product, material, parentBuildOfMaterial });
      ...
      expect(service.calculateLevel).toHaveBeenCalledWith(parentBuildOfMaterial);
      ...
      expect(mockBuildOfMaterialsRepository.save).toHaveBeenCalledWith(buildOfMaterial);
    });
  ...
  describe('remove', () => {
    it('deve remover o BuildOfMaterial e seus filhos recursivamente', async () => {
        ...
        const childBuildOfMaterial = { id: '456', childMaterials: [], product, material } as BuildOfMaterial;
        const parentBuildOfMaterial = { id: '123', childMaterials: [childBuildOfMaterial], product, material } as BuildOfMaterial;
        ...
        expect(mockBuildOfMaterialsRepository.remove).toHaveBeenCalledWith(childBuildOfMaterial);
        expect(mockBuildOfMaterialsRepository.remove).toHaveBeenCalledWith(parentBuildOfMaterial);
    });
    ...
    it('deve lançar InternalServerErrorException se houver erro ao remover', async () => {
      const product: Product = { id: '123', description: 'Test Product', code: 'P123', isActive: true };
      const material: Material = { id: '456', description: 'Test Material', code: 'M456', qtd: 100 };
      const buildOfMaterial = { id: '123', childMaterials: [], product, material } as BuildOfMaterial;
      ...
      await expect(service.remove('123')).rejects.toThrow(InternalServerErrorException);
    });
  ...
  describe('getByProductAndLvl', () => {
    it('deve retornar os BuildOfMaterials pelo produto e nível', async () => {
      ...
      const product: Product = { id: 'prod123', description: 'Product Test', code: 'P123', isActive: true };
      ...
      expect(mockProductsService.validateExistingProduct).toHaveBeenCalledWith('prod123');
    ...
    it('deve lançar NotFoundException se o produto não for encontrado', async () => {
      mockProductsService.validateExistingProduct.mockRejectedValue(new NotFoundException());
  
      await expect(service.getByProductAndLvl('prod123', 1)).rejects.toThrow(NotFoundException);
    });
    ...
    it('deve lançar InternalServerErrorException se o repositório find falhar', async () => {
      ...
      mockProductsService.validateExistingProduct.mockResolvedValue(product);
      mockBuildOfMaterialsRepository.find.mockRejectedValue(new Error('Database error'));
  
      await expect(service.getByProductAndLvl('prod123', 1)).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe('getDirectChildren', () => {
    it('deve retornar os filhos diretos de um BuildOfMaterial', async () => {
      ...
      expect(mockBuildOfMaterialsRepository.findOne).toHaveBeenCalledWith({ where: { id: '123' } });
      expect(mockBuildOfMaterialsRepository.find).toHaveBeenCalledWith({ where: { parentBuildOfMaterial: { id: '123' } }, relations: ['material'] });
    ...
    it('deve lançar NotFoundException se o BuildOfMaterial não for encontrado', async () => {
      ...
      await expect(service.getDirectChildren('123')).rejects.toThrow(NotFoundException);
    });
  });

  describe('validateCreate', () => {
    it('deve validar o produto, material e parentBuildOfMaterial', async () => {
      const createDto = { productId: 'prod123', materialId: 'mat123', parentBuildOfMaterialId: 'bom123', qtd: 10 };
      const product: Product = { id: 'prod123', description: 'Product Test', code: 'P123', isActive: true };
      const material: Material = { id: 'mat123', description: 'Material Test', code: 'M123', qtd: 50 };
      const parentBuildOfMaterial: BuildOfMaterial = { id: 'bom123', lvl: 1, qtd: 10, product, material, parentBuildOfMaterial: null, childMaterials: [] };

      mockProductsService.validateExistingProduct.mockResolvedValue(product);
      mockMaterialsService.validateExistingMaterial.mockResolvedValue(material);
      mockBuildOfMaterialsRepository.findOne.mockResolvedValue(parentBuildOfMaterial);

      const result = await service.validateCreate(createDto);

      expect(mockProductsService.validateExistingProduct).toHaveBeenCalledWith('prod123');
      expect(mockMaterialsService.validateExistingMaterial).toHaveBeenCalledWith('mat123');
      expect(mockBuildOfMaterialsRepository.findOne).toHaveBeenCalledWith({ where: { id: 'bom123' } });
    ...
    it('deve lançar NotFoundException se o parentBuildOfMaterial não for encontrado', async () => {
      ...
      await expect(service.validateCreate(createDto)).rejects.toThrow(NotFoundException);
    });
